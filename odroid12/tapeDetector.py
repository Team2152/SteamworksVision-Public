...
class tapeDetector():
	...
	def detectTape(self, imageIn):
		frame = cv2.cvtColor(imageIn[100:], cv2.COLOR_BGR2HSV)
		...
		if(self.tape == "peg"):
			cv2.circle(frame, (20,20), 10, (0,255,255), 5)
		elif(self.tape == "boiler"):
			cv2.circle(frame, (20,20), 10, (127,255,0), 5)
		else:
			cv2.circle(frame, (20,20), 10, (255, 0, 0), 5)
		if len(cnts) > 0:
			...
			for cnt in cnts[:count]:
				...
				if cnt is not None and cv2.contourArea(cnt) > 50:
					...
					if i == 2:
						...
						yAngle = m.degrees(m.atan((pegY+100-(res[1]/2-0.5))/adj2))
						"""
						if (h != 0):
						...
						if(pegYs[3]>130 or pegYs[0] > 130 or pegYs[1] > 130 or pegYs[2] > 130):
							parallax = w
							distance = 2792.11/w
						else:
							parallax = h
		...
		frame = np.concatenate((imageIn[:101], frame), axis=0)
		return frame, mask
